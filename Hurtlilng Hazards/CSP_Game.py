...
game_over_state = False # New state variable to track if game over screen is displayed
...
def move_hazards():
    ...
    for hazard in fallingObjects:
        ...
        # Hazard moves off-screen, reset it and give score
        if current_y < -320:
...
def game_over(reason):
    ...
    global game_is_running, game_over_state
    game_is_running = False
    game_over_state = True # Set state to indicate game over screen is active
    ...
    # Add the "Press SPACE" instruction
    game_over_text = (
        f"{final_message}\n\n"
        f"{final_score_line}\n"
        f"{final_time_line}\n\n"
    ...
    score_guy.write(game_over_text, 
...
def reset_game():
    """Resets all game variables and turtles for a new game."""
    global score, stopwatch, fallingObjects, game_over_state
    ...
    # Clean up falling objects (destroy them to free memory/prevent visual ghosting)
    for hazard in fallingObjects:
        hazard.hideturtle() # Hide first
        hazard.clear()      # Clear any drawings
    fallingObjects.clear()  # Empty the list
    
    # Reset player and flame position/appearance
    player.goto(player_start_x, player_start_y)
    player.shape("turtle")
    flameTurtle.goto(player_start_x, player_start_y + flame_offset_y)
    flameTurtle.showturtle()
    
    # Clear game over/start message and display initial score position
    score_guy.clear()
...
def game_started():
    """Initializes and starts the main game loops, handling the reset if necessary."""
    global game_is_running
    
    if not game_is_running:
        if game_over_state:
            reset_game() # Perform full reset if we're coming from a Game Over screen
...
wn.onkeypress(game_started, "space") # Use space bar to start/restart the game
wn.listen()
